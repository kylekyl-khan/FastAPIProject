...
## 8. 使用內建演示資料瀏覽 4000 人規模的組織

為方便快速展示多層級組織的操作流程，專案額外提供一組使用 SQLite 建立的演示資料庫。啟動伺服器後，FastAPI 會自動在專案根目錄建立 `demo_org.db`，並填入多層級的示範組織與人員。

1. 啟動服務後，造訪 `http://127.0.0.1:8000/demo`。
2. 左側為可展開/收合的組織樹，支援即時搜尋。
3. 中央面板會顯示所選組織的層級資訊與直屬成員，可直接勾選單一人員。
4. 右側摘要區會統計已勾選的組織與人員，若勾選上層組織，所有下屬組織與成員會自動納入。

> 若要改用真實資料，只需維持原本的 SQL Server 設定與資料表結構；演示頁面同樣會透過 API 從資料庫載入實際內容。

---

## 9.（可選）啟用 HTTPS 以配合 Outlook 外掛
